...
function computeResilience(input) {
  const {
    ...
    varExp = 0,
    insurance = 0,
    bnplDebt = 0,      // monthly BNPL commitment (NOT total debt)
    dependents = 0,
    income = 0,
  } = input;
  ...
  // bnplDebt is monthly commitment → adds to monthly burn, does NOT reduce savings
  const monthlyBurn = fixedExp + varExp + bnplDebt + dependents * 300;
 
  // netAssets = cash savings + insurance buffer (insurance is NOT subtracted)
  const netAssets = savings + insurance * 0.5;
 
  const scoreMonths = monthlyBurn > 0 ? Math.max(0, netAssets / monthlyBurn) : 0;
 
  // Flutter displays score OUT OF 10, not raw months
  // Score = weighted average of the same 4 metrics Flutter calculates locally
 
  // Metric 1: Emergency Buffer — savings vs monthly expenses (not burn)
  const emergencyMonths = fixedExp > 0 ? savings / fixedExp : 0;
  const emergencyScore = emergencyMonths >= 6 ? 10
    : emergencyMonths >= 4 ? 8
    : emergencyMonths >= 3 ? 7
    : emergencyMonths >= 2 ? 5.5
    : emergencyMonths >= 1 ? 4
    : emergencyMonths > 0  ? 2.5
    : 1;
 
  // Metric 2: Debt Pressure — bnplDebt as % of income
  const bnplRatio = income > 0 ? bnplDebt / income : 0;
  const debtScore = bnplRatio <= 0    ? 10
    : bnplRatio <= 0.10 ? 8.5
    : bnplRatio <= 0.20 ? 6.5
    : bnplRatio <= 0.30 ? 4
    : 2;
 
  // Metric 3: Monthly Saving Capacity — (income - fixedExp - varExp - bnplDebt) / income
  const monthlySavingsCapacity = income - fixedExp - varExp - bnplDebt;
  const savingsCapacityRate = income > 0 ? monthlySavingsCapacity / income : 0;
  const savingsCapacityScore = savingsCapacityRate >= 0.30 ? 10
    : savingsCapacityRate >= 0.20 ? 8
    : savingsCapacityRate >= 0.10 ? 6
    : savingsCapacityRate >= 0.05 ? 4
    : savingsCapacityRate >= 0    ? 2.5
    : 1;
 
  // Metric 4: Goal Progress — savings toward 6-month emergency fund target
  const savingsGoalTarget = fixedExp * 6;
  const goalProgress = savingsGoalTarget > 0
    ? Math.min(savings / savingsGoalTarget, 1)
    : 0;
  const goalScore = goalProgress >= 1    ? 10
    : goalProgress >= 0.75 ? 8
    : goalProgress >= 0.50 ? 6.5
    : goalProgress >= 0.25 ? 4.5
    : goalProgress > 0     ? 3
    : 1.5;
 
  // Weighted average (matches Flutter's equal-weight average)
  const rawScore = (emergencyScore + debtScore + savingsCapacityScore + goalScore) / 4;
  const scoreOut10 = round(Math.min(rawScore, 10));
 
  // Flutter stress test toggle: score * 0.77
  const stressTestScore = round(scoreOut10 * 0.77);
 
  // Survival months (for description text) — based on emergency buffer only
  // Survival months = emergency buffer months (based on fixedExp only)
  const survivalMonths = round(emergencyMonths);
  const survivalDays = Math.round(emergencyMonths * 30);
  const stressDays = Math.round(emergencyMonths * 0.77 * 30);
  ...
  const debtToIncomeRatio = income > 0 ? round(bnplRatio * 100) : 0;
  const savingsRate = income > 0 ? round(savingsCapacityRate * 100) : 0;
 
  // Emergency fund description
  const emergencyFundDesc = emergencyMonths < 1
    ? `Covers ${survivalDays} days — critical, target: 6 months`
    : `Covers ${round(emergencyMonths)} months — target: 6 months`;
 
  // Goal progress description
  const goalDesc = savingsGoalTarget > 0
    ? `${round(goalProgress * 100)}% toward a 6-month emergency fund (RM${round(savingsGoalTarget)})`
    : 'Set a savings goal to track resilience progress';
 
  // Debt description
  const dtiDesc = debtToIncomeRatio === 0
    ? 'No BNPL debt — excellent position'
    : `${debtToIncomeRatio}% of income — ${debtToIncomeRatio < 20 ? 'manageable' : 'high risk'}`;
 
  // Monthly savings description
  const savingsDesc = savingsCapacityRate <= 0
    ? 'Overspending — no monthly saving capacity'
    : `${savingsRate}% of income available to save — ${savingsRate >= 20 ? 'great!' : 'aim for 20%'}`;
 
 
  const breakdown = [
    { icon: '🛟', title: 'Emergency Buffer',        score: round(emergencyScore),        description: emergencyFundDesc },
    { icon: '💳', title: 'Debt Pressure',           score: round(debtScore),             description: dtiDesc },
    { icon: '💸', title: 'Monthly Saving Capacity', score: round(savingsCapacityScore),  description: savingsDesc },
    { icon: '🎯', title: 'Goal Progress',           score: round(goalScore),             description: goalDesc },
  ];
